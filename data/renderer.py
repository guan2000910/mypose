...
def orthographic_matrix(left, right, bottom, top, near, far): #生成正交投影矩阵
    return np.array((
...
class ObjCoordRenderer: #渲染模型坐标到图像上
    ...
    def read(self): #读取渲染结果 返回RGB和透明度
    ...
    @staticmethod
    def extract_mask(model_coords_img: np.ndarray): #返回从模型坐标图像中提取到的掩码
        return model_coords_img[..., 3] == 255

    def denormalize(self, model_coords: np.ndarray, obj_idx: int): #还原模型坐标
