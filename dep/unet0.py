...
class ResNetUNet(nn.Module):
    ...
    def __init__(self, n_class, feat_preultimate=64, n_decoders=1):
       ...
       #加入convnext：
        self.ctlayer0 = nn.Sequential(self.ct_layers[0]) #size=(N, 128, x.H, x.W)
        self.ctlayer1 = self.ct_layers[1]
        self.ctlayer2 = nn.Sequential(self.ct_layers[2]) #size=(N, 256, x.H, x.W)
        self.ctlayer3 = self.ct_layers[3]
        self.ctlayer4 = nn.Sequential(self.ct_layers[4])  #size=(N, 512, x.H, x.W)
    ...
    def forward(self, input, decoder_idx=None):
        ...
        layerct1 = self.ctlayer1(layerct0)
        layerct2 = self.ctlayer2(layerct1)
        layerct3 = self.ctlayer3(layerct2)
        ...
        layercts = [layerct0, layerct1, layerct2, layerct3, layerct5]
        ...
        for i, dec_idx in enumerate(decoder_idx):
            ...
            x = decoder['layer4_1x1'](layer4_fused[batch_slice])
            simam_module(x)
            x = self.upsample(x)
