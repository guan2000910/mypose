...
with open('data/detection_results/gdrnppdet-pbrreal_tless-test_e112ecb4-7f56-4107-8a21-945bc7661267.json', 'r') as f:
    data = json.load(f)

# 提取所需的字段并保存为 NumPy 数组
scene_ids = np.array([item['scene_id'] for item in data])
view_ids = np.array([item['image_id'] for item in data])
obj_ids = np.array([item['category_id'] for item in data])
bboxes = np.array([item['bbox'] for item in data])
scores = np.array([item['score'] for item in data])
times = np.array([item['time'] for item in data])


debug = args.debug
mask_all = np.zeros(len(scores), dtype=bool)
for scene_id in sorted(inst_count.keys()):
    inst_count_scene = inst_count[scene_id]#获取与场景ID相关的inst_count
    ...
    for view_id in sorted(inst_count_scene.keys()):
        ...
        for obj_id in sorted(inst_count_view.keys()):
            ...
            filtered_mask = scores[arg_mask] >= 0.1
            arg_mask = arg_mask[filtered_mask]
