...
import os
import sys
sys.path.insert(0,r'/home/guanjian/surfemb')
...
from surfemb.data import obj, instance
...
def main():
    ...
    parser.add_argument('--max-steps', type=int, default=700_000)
    ...
    # datasets
    auxs = model.get_auxs(objs, args.res_crop) #返回包含辅助信息的字典auxs
    ...
    trainer = pl.Trainer(
        ...
        logger=logger, gpus=args.gpus, max_steps=args.max_steps, #accelerator="dp", 
        callbacks=[
...
""" trainer = pl.Trainer(
    ...
    val_check_interval=min(1., n_valid / len(data) * 50)
)

# 检查是否有要恢复的检查点路径，并在需要时更新模型路径
if args.ckpt:
    model_ckpt_cb.ckpt_path = args.ckpt
